...
if ($msgType == 'text') {
	 ...
	 if($_count>0){
		 if("Y" == $content  || 'y' == $content){
	        ...
	        replyTextAndImg($sendUserId, $developerId, '炮制＃马上你就红＃祝福海报', $description, 'http://115.29.49.54/image/'.$userInfo['picName'], 'http://tongyi.mei94.com/page?id='.urlencode($userInfo['picName']));
