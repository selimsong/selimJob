...
if ($msgType == 'event') {
	if ($xml->Event == 'subscribe') {
	        ...
	        replyTextAndImg($sendUserId, $developerId, '#心意自造#潮卡送到', $description, 'http://115.29.49.54/activity.jpg?v=123', 'http://tongyi.mei94.com/page');
	}
	exit();
}
	 ...
	 if($_count>0){
		 if("Y" == $content  || 'y' == $content){
            ...
            if(($geo['height'] - $geo['width']) > 10){
			   $imageBig = new Imagick('chan-buttom.jpg');
			   $text_x = 120;
			   $text_y = 820;
			   $degree = -9;
			}else{
			   $imageBig = new Imagick('zhen-buttom.jpg');
			   $text_x = 110;
			   $text_y = 724;
			   $degree = -7;
			}
